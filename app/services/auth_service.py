...
async def register_user(email: str, password: str, consent: bool = False):
    # check exists
    ...
    res = await database.execute(models.users.insert().values(email=email, hashed_password=hashed, is_active=True, is_verified=False, consent=consent, role='student'))
    user = await database.fetch_one(models.users.select().where(models.users.c.id == res))
    # send verification email
    token = create_refresh_token(str(user.id))
    verify_link = f"{settings.FRONTEND_ORIGIN}/verify-email?token={token}"
    await email_service.send_email(email, "Verify your account", f"Click to verify: {verify_link}")
    await audit_service.log(user_id=user.id, action="user.register", meta={})
    return user


async def authenticate_user(email: str, password: str = None, user_agent: str = None, ip: str = None, oauth_user: dict = None):
    """
    ...
    if oauth_user:
        ...
        if not user:
            await audit_service.log(None, "auth.failure", {"email": email, "reason": "not_found"})
            return None
        if not user["hashed_password"]:
            return None
        if not verify_password(password, user["hashed_password"]):
            await audit_service.log(user["id"], "auth.failure", {"reason": "invalid_password"})
            return None

    # create session
    session = await session_service.create_session(user_id=user["id"], user_agent=user_agent, ip=ip)
    ...
    await audit_service.log(user["id"], "auth.success", {"session_id": session})
...
async def refresh_tokens(refresh_token: str):
    ...
    await audit_service.log(user_id, "auth.refresh", {})
    return {"access_token": access, "refresh_token": refresh, "token_type": "bearer"}
...
async def logout(refresh_token: str):
    ...
    if not payload:
        return False
    user_id = payload.get("sub")
    await audit_service.log(user_id, "auth.logout", {})
    return True
